...
get_vpn_string() {
    local vpn_string
    if get_vpn_status -s "$TEST_MODE"; then
        vpn_string="$TEST_MODE"
    fi
    if get_vpn_status -s "$PRODUCTION_MODE"; then 
        vpn_string="$PRODUCTION_MODE"
    fi
    echo "$vpn_string"
    return 0
}
...
get_vpn_ip(){
    ...
    while [ "$TIMEOUT" -gt 0 ]; do
        if [ "$(get_vpn_ip_raw "$target_interface" 2>/dev/null)" != "" ]; then
            get_vpn_ip_raw "$target_interface" 2>/dev/null
            return 0
        fi
        sleep "$TIMEINTERVAL"
        TIMEOUT=$((TIMEOUT-1))
...
start_vpn(){
    ...
    while getopts 'i' short_opt; do
        case $short_opt in
            ...
            *)
                ...
                echo "Start CSCC VPN"
                echo ""
                echo "  -i, --ignore          ignore current VPN status"
                exit $ERR_GENERIC
        esac
        shift
    done
    ...
    if [[ -n "$vpn_string" ]]; then
        warn "$vpn_string VPN is stopped, check whether wants' to start" `
            ` "$target_vpn_type VPN..."
    fi
    if [[ -z $ignore_enabled ]]; then
        read -r -p "$target_vpn_type VPN service is not running, do you "`
            `"want to start it? (Y/n) - " answer 
        if [ "$answer" != "${answer#[Nn]}" ]; then
            info "answer is \`$answer\` VPN service would not not started, "`
                `"exiting..."
            exit 0
        else
    ...
    if [ "$target_vpn_type" == "$PRODUCTION_MODE" ]; then
        ...
        # FIXME: refactor this code, follwoing is duplicated

        # add route to prevent exitnode ruins whole routing table
    ...
    sudo resolvectl domain $target_interface 'test.cc.cs.nctu.edu.tw' 'cc.cs.nctu.edu.tw'
